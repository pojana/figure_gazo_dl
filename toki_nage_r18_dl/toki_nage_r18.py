...
syuum_path = 'H:\\いろいろ\\figs\\朱鷺長のフォトブログ(R18)\\'
moto_url = 'http://blog.livedoor.jp/toki_naga-r18/'
...
def scrape(url, save_path):
    ...
    title = soup.find('title').text.replace('【R18】', '').replace('【フィギュアレビュー】 : 朱鷺長のフォトブログ(R18)', '')
    title = re.sub(r'[\\|/|:|?|.|"|<|>|\|\n|]', '-', title)
    title = title.strip()
    ...
    src_iml = soup.find("div", class_='article-body-inner')
    src_iml = src_iml.find_all('a', target='_blank')
