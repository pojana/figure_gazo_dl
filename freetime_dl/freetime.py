...
def scrape(url, save_path):
    ...
    title = soup.find('title').text.replace('レビュー', '').replace('FREETIME (R18)', '')
    title = re.sub(r'[\\|/|:|?|.|"|<|>|\|\n|*|]', '-', title)
    title = title.replace('-', '').strip()
    print(pathlib.Path(save_path + title))

    src_iml = soup.find("div", class_='clearfix').find("div", class_='clearfix')
...
def get_download_page_list(url):
    ...
    next = soup.find('ul', class_="pagination").find_all('li')[-2].find('a')
