...
syuum_path = 'H:\\いろいろ\\figs\\hobbyholic\\'
moto_url = 'http://hobbyholic.org/tag/r18'
...
def scrape(url, save_path):
    ...
    title = soup.find('title').text.replace('製品レビュー', '').replace('-', '')
    title = re.sub(r'[\\|/|:|?|.|"|<|>|\|\n|]', '-', title)
    title = replace_fileName(title)
    title = title.replace('［', '').replace('］', ' ').replace('-', '').strip()
    print(pathlib.Path(save_path + title))

    src_iml = soup.find("div", class_="post")
...
def get_download_page_list(url):
    ...
    url_list = [s.get('href') for s in soup.find_all('a', class_='more-link')]
    # print(url_list)
