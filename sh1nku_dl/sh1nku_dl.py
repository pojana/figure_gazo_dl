...
syuum_path = 'H:\\いろいろ\\_downloader\\fig_down\\sh1nku\\'
moto_url = 'http://sh1nku.blog.fc2.com/archives.html'
...
def scrape(url, save_path):
    ...
    img_list = [s.get('src') for s in entry.find_all('img')]
...
def get_download_page_list(url):
    ...
    src = soup.find('ul', class_='list_body')
    # print(src)
    # url_list = [s.find('a').get('href') for s in src]
    url_list = [s.get('href') for s in src.find_all('a')]
